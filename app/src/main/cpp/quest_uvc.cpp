...
static void stop_session(Session* s){
    if(!s)return;bool was_running=s->running.exchange(false);if(!was_running&&!s->event_thread.joinable()&&s->transfers.empty())return;for(auto* t:s->transfers)libusb_cancel_transfer(t);if(s->event_thread.joinable())s->event_thread.join();
